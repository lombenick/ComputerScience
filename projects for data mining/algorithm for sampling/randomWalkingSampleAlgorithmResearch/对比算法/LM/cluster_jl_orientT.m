...
% Inputs : 
% M : weight matrix带权矩阵
% s : 1 = Recursive computation为1时递归计算
%   : 0 = Just one level computation为0时只一层计算
% self : 1 = Use self weights为1时使用自身权重
%        0 = Do not use self weights为0时不使用自身权重
% debug   : 1 = outputs some debug messages为1时输出调试信息
% verbose : 1 = outputs some messages为1时输出信息
%
% Output :
% COMTY, structure with the following information
% for each level i :
%   COMTY.COM{i} : vector of community IDs (sorted by community sizes)社区ID
%   COMTY.SIZE{i} : vector of community sizes社区大小
%   COMTY.MOD(i) : modularity of clustering模块度
%   COMTY.Niter(i) : Number of iteration before convergence收敛前的迭代次数
%
function [COMTY ending] = cluster_jl_orientT(M,s,self,debug,verbose)
...
% Main loop
K = sum(M); % Sum of wieght incident to node i M的每列之和
SumTot = sum(M);
SumIn = diag(M); % Sum of weight inside community i  初始各社区的权重之和
COM = 1:S(1); % Community of node i  初始所有社区
for k=1:N
  Neighbor{k} = find(M2(k,:));   %计算各个节点的邻居节点
end
...
if (s == 1)
  ...
  while 1
    ...
    [COMt e] = cluster_jl_orientT(Mnew,0,self,debug,verbose);
    if (e ~= 1)
      COMfull = zeros(1,N);
      COMcur = COMt.COM{1};
      for p=1:Ncom
