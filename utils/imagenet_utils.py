...
def get_submodules_from_kwargs(kwargs):
    _backend = kwargs.get('backend', _KERAS_BACKEND)
    _layers = kwargs.get('layers', _KERAS_LAYERS)
    _models = kwargs.get('models', _KERAS_MODELS)
    _utils = kwargs.get('utils', _KERAS_UTILS)
    for key in kwargs.keys():
        if key not in ['backend', 'layers', 'models', 'utils']:
            raise TypeError('Invalid keyword argument: %s', key)
    return _backend, _layers, _models, _utils
