...
#   数据加载器
class FacenetDataset(tf.keras.utils.Sequence):
    def __init__(self, input_shape, dataset_path, num_train, num_classes, batch_size, flip_signal=False,
                 rotate_signal=False, color_signal=False, backbone='Mobilenet'):
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.batch_size):
            ...
            image = self.get_random_data(image, [self.image_height, self.image_width], flip_signal=self.flip_signal,
                                         rotate_signal=self.rotate_signal, color_signal=self.color_signal)
        ...
        if self.backbone == 'VGG' or self.backbone == 'Inception V2':
    ...
    # 数据增强
    def get_random_data(self, image, input_shape, jitter=.1, hue=.1, sat=1.3, val=1.3,
                        flip_signal=False, rotate_signal=False, color_signal=False):
        ...
        rotate = self.rand() < .5
        if rotate and rotate_signal:
            angle = np.random.randint(-10, 10)
            a, b = w / 2, h / 2
            M = cv2.getRotationMatrix2D((a, b), angle, 1)
            image_data = cv2.warpAffine(np.array(image_data), M, (w, h), borderValue=[128, 128, 128])
...
class LFWDataset:
    def __init__(self, _dir, pairs_path, input_shape, batch_size):
        super(LFWDataset, self).__init__()
        self.input_shape = input_shape
        self.pairs_path = pairs_path
        self.batch_size = batch_size
        self.validation_images = self.get_lfw_paths(_dir)
